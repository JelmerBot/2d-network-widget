...
@widgets.register
class Widget2D(widgets.DOMWidget):
    """The widget."""

    _view_name = Unicode("NetworkWidget2DView").tag(sync=True)
    _model_name = Unicode("NetworkWidget2DModel").tag(sync=True)
    _view_module = Unicode("network_widget_2d").tag(sync=True)
    _model_module = Unicode("network_widget_2d").tag(sync=True)
